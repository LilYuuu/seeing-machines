...
void ofApp::draw(){
    ...
    // Create a mask that makes the background dark
    ofPixels diffPixels = difference.getPixels();
    for (int i = 0; i < diffPixels.size(); i++) {
        if (diffPixels[i] <= 0) {
            color.getPixels()[i * 3] = 0; // Set the red channel to zero
            color.getPixels()[i * 3 + 1] = 0; // Set the green channel to zero
            color.getPixels()[i * 3 + 2] = 0; // Set the blue channel to zero
        }
    }
//    color.update();
